...
class API
{
    ...
    public function CallMethod($method, $request)
    {
        ...
        $keys = array_keys($actionData);
        //$args = array();
        //$args = array_values($actionData);
        $ReflectionMethod =  new ReflectionMethod($this, $request);
        ...
        if ($keyFound) {
            foreach ($paramNames as $paramName) {
                $args[] = $actionData[$paramName];
                /*if (array_key_exists($paramName, $actionData)) {
                    $args[] = $actionData[$paramName];
                } else {
                    $args[] = $actionData[$keys[$pos]];
                }
                $pos++;*/
            }
        } else {
